...
describe("TripCard", () => {
  ...
  it("케밥 메뉴의 상세보기는 그 자리에서 펼치지 않고 '내 여행' 페이지로 가는 링크다", async () => {
    render(<TripCard item={stub} onRemove={vi.fn()} />);

    await userEvent.click(screen.getByRole("button", { name: "더보기" }));

    expect(screen.getByRole("menuitem", { name: "상세보기" })).toHaveAttribute(
      "href",
      "/itineraries",
    );
    expect(screen.queryByText("생성된 일정")).not.toBeInTheDocument();
  });
