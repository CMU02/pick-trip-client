...
1. **육안 확인 (2026-08-30, dev :3000)** — `.env.local`에 KAKAO 키 다 있음. 확인됨: 폭 `max-w-7xl`
   헤더 정렬, 지도·마커 렌더, 갤러리 5장→`+2`·6장→`+3`·7장→`+4`, `+N` 라이트박스(4열 격자)·사진
   선택 시 대표 전환 후 닫힘·Esc·콘솔 에러 없음. **미확인**: 1024px 2열 전환·`<aside>` sticky·좌표
   없음 폴백·모바일 패널 스택. **근처 콘텐츠 3지역 확인 완료** — 백엔드가 `/nearby`를 TourAPI 위치
   검색 폴백(`source:"TOURAPI"`)으로 고쳐 200 응답. 하동 5곳·예천 6곳 전부 3칸, 영주 6곳 중 4곳
   3칸·1곳 2칸(노계서원, 2칸 좌측정렬·안 늘어남)·1곳 0건(김진호가옥 → 섹션 숨김, 레이아웃 정상).
   거리 표기 m/km 양쪽 확인(`약 140m`·`약 600m`·`약 3.6km`·`약 4.3km`). 카드 클릭 이동,
   `?from=explore` 전파, 지역 경계 넘는 근처(영주 콘텐츠가 예천 항목 반환) 정상. 긴 이름은 `truncate`.
   응답의 `source` 필드는 프론트 타입에 없지만 무시되어 무해.
   ※ 백엔드 이슈 후보: 영주 `2600301`이 `radiusKm=5`엔 0건, `radiusKm=20`엔 10건이고 그중 최근접이
   3.71km(5km 이내) — radius 필터가 반경 5에서 정상 동작 안 하는 듯. 백엔드팀 확인 필요.
...
5. **근처 콘텐츠** — 백엔드 radius 필터 버그 후보(위 §1 ※) 백엔드팀에 전달됨. 외곽 콘텐츠에서
   0~2건 나올 때 UX(반경 확대 / 섹션 숨김 유지) 재검토 여지.
