...
1. **육안 확인 (2026-08-30, dev :3000)** — `.env.local`에 KAKAO 키 다 있음. 확인됨: 폭 `max-w-7xl`
   ...
   없음 폴백·모바일 패널 스택. **근처 콘텐츠 확인 완료** — 백엔드가 `/nearby`를 TourAPI 위치 검색
   폴백(`source:"TOURAPI"`)으로 고쳐 200 응답. 3칸 카드(이미지·이름·`약 140m`/`약 600m` 등 거리
   표기·카테고리) 렌더, 카드 클릭 이동, `?from=explore` 링크 전파 전부 정상. 응답에 프론트 타입에
   없는 `source` 필드가 있으나 무시되어 무해.
...
5. **근처 콘텐츠** — 확인한 케이스(하동 5곳)는 모두 3건 이상 반환됨. 시골/외곽 콘텐츠에서
   `radiusKm` 기본 5km 결과가 0~2건인 경우 UX(반경 확대 or 섹션 숨김) 재검토.
