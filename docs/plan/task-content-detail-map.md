...
1. **육안 확인 (2026-08-30, dev :3000)** — `.env.local`에 KAKAO 키 다 있음. 확인됨: 폭 `max-w-7xl`
   헤더 정렬, 지도·마커 렌더, 갤러리 6장→`+3`·7장→`+4`, `+N` 라이트박스(4열 격자)·사진 선택 시
   대표 전환 후 닫힘·Esc·콘솔 에러 없음. **미확인**: 1024px 2열 전환·`<aside>` sticky·좌표 없음
   폴백·모바일 패널 스택. **근처 콘텐츠는 확인 불가** — 실행 중 백엔드(main)에 `/nearby`가 없어
   404(엔드포인트는 백엔드 `feat/66`에만 있음, 백엔드 main 미머지). 프론트는 404를 정상 흡수해
   섹션을 숨김(레이아웃·콘솔 정상) 확인 완료. 백엔드 `feat/66` 실행 후 실제 3칸 확인 필요.
