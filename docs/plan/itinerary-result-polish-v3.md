...
## B. "담은 콘텐츠" 리스트 높이 제한 — `PreGenerateView`

- 3번 섹션("담은 콘텐츠")의 그룹 목록을 감싸는 컨테이너에
  `max-h-[420px] overflow-y-auto`(+ 스크롤 시 잘림 방지 `pr-1`, 스크롤바 여백) 부여.
- 항목이 적으면 `max-h`가 영향을 주지 않아 기존과 동일.
- 빈 상태(담은 콘텐츠 없음) 블록은 스크롤 컨테이너 밖(현행 유지).
- 헤더의 "콘텐츠 더 담기 →" 링크와 개수 뱃지는 스크롤 영역 밖(현행 유지).
